...
import com.haxepunk.Graphic;
import com.haxepunk.utils.Strings;
    ...
    public function getGlyphs (text :String) :Array<Glyph>
    {
        ...
        for (ii in 0...text.length) {
            ...
            if (glyph != null) {
               ...
               HXP.log("Requested a missing character from font"+
        ...
        for (keyword in parser.keywords()) {
            ...
            case "page":
             ...
             //   pages.set(pageId, _pack.getTexture(basePath + file.removeFileExtension()));
			    pages.set(pageId, HXP.engine.game.getTexture("gfx/"+file));    //_pack.getTexture(basePath + file.removeFileExtension()));
...
class Glyph
{
     ...
     */
    public function draw (m:Matrix, destX :Float, destY :Float)
    {
        // Avoid drawing whitespace
        if (clip.width > 0) 
		{
			Game.spriteBatch.drawEntity(page, m, clip, destX + clip.offsetX, destY + clip.offsetY, false, false,1,1,1,1, BlendMode.NORMAL);
...
class TextLayout
{
    ...
    public function draw (m:Matrix,p:Point)
    {
        var y = 0.0;
        var ii = 0;
        var ll = _glyphs.length;
