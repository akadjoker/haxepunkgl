...
class HXP
{
	 ...
	 */
	public static inline var VERSION : String = "gl by djoker";
	...
	public static inline function point_direction(x1:Float,y1:Float,x2:Float,y2:Float):Float
	{
		var dx, dy:Float;
		dx = x1 - x2;
		dy = y1 - y2;
		return Math.atan2( -dy, dx) * (180 / 3.14) + 180;
		
		
		
	}
	public static inline function lengthdir_x(len:Float,dir:Float):Float
	{
		return Math.cos(dir * Math.PI / 180) * len;
	}	
	public static inline function lengthdir_y(len:Float,dir:Float):Float
	{
		return -Math.sin(dir * Math.PI / 180) * len;
	}	
	public static inline function length_dir_x(x:Float,len:Float,dir:Float):Float
	{
		return x + Math.cos(dir * Math.PI / 180) * len;
		
		
	}	
	public static inline function length_dir_y(y:Float,len:Float,dir:Float):Float
	{
	...
	public static function resize(width:Int, height:Int)
	{
		...
		HXP.screen.resize();
		if (HXP.screen.needsResize) HXP.screen.needsResize = false;
...
public static function computeVelocity(Velocity:Float, Acceleration:Float, Drag:Float, Max:Float,deltaTime:Float):Float
	{
		if (Acceleration != 0)
		{
...
public static inline function distAbs(x1:Float,  x2:Float ):Float
	...
	public static var debugEntityBound:Bool = false;
	public static var debugDrawMask:Bool = false;
