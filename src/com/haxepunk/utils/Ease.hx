...
class Ease
{
	...
	public static function elastic(a:Float):Float
	{
		if (a <= 0.5) {
				a *= 2;
				return Math.pow(elastic_value, elastic_power * (a - 1)) * Math.sin(a * 20) * 1.0955/ 2;
			}
			a = 1 - a;
			a *= 2;
			return 1 - Math.pow(elastic_value, elastic_power * (a - 1)) * Math.sin((a) * 20) * 1.0955 / 2;
	}
	public static function elasticIn(a:Float):Float
	{
		return Math.pow(elastic_value, elastic_power * (a - 1)) * Math.sin(a * 20) * 1.0955;
	}
	public static function elasticOut(a:Float):Float
	{
	       a = 1 - a;
			return (1 - Math.pow(elastic_value, elastic_power * (a - 1)) * Math.sin(a * 20) * 1.0955);
	}
	...
	private static inline var elastic_value:Float = 2;
	private static inline var elastic_power:Float = 10;
