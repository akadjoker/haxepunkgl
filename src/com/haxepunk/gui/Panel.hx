...
class Panel extends Control
{
	 ...
	 */
	public function new(x:Float = 0, y:Float = 0, width:Int = 48, height:Int = 48, ?skin:Texture)
	{
		children = new Array<Control>();
		
		super(x, y, width, height, skin);
		graphic = new NineSlice(width, height, new Clip(48, 0, 16, 16), _skin);
	...
	public function add(control:Control, x:Float = 0, y:Float = 0, layer:Int = 0):Control
	{
		...
		if (this.world == HXP.scene) { HXP.scene.add(control); }
	...
	private function get_hide():Dynamic
	{
		...
		return Reflect.makeVarArgs(function (objs:Array<Dynamic>):Dynamic
		{
			...
			for (i in 0...objs.length)
			{
				...
				return me.children[index].visible;
			}
			return null;
		});
	}
